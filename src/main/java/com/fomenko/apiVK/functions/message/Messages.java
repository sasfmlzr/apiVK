package com.fomenko.apiVK.functions.message;
...
import static com.fomenko.apiVK.object.StatisticsVariable.countSendMessage;
import static com.fomenko.apiVK.object.StatisticsVariable.timeItogoSendMessage;
    ...
    public void vksendImageMessages(UserActor actor, List<Dialog> messagesList, VkApiClient vk) throws ClientException, ApiException {
      ...
      //      System.out.println(files[1].exists());  //файл существует
        ...
        //-----------------отправить сообщения без разницы кому, отметка о том, что сообщение прочитано и отслеживание времени, затраченного на отправку------------------------//
        public void vkSendMessage(GroupActor actor, String message, List<Dialog> messagesList, VkApiClient vk) throws ClientException, ApiException {
            Boolean readState =  messagesList.get(0).getMessage().isReadState();            //статус прочтения
            if (!readState) {
                long timeStartSendMessage=      System.currentTimeMillis();
                int chatID, userID;
        ...
        //-----------------отправить сообщение пользователюу---------------------------//
        public void vkSendMessageUser(GroupActor actor, String message, int userID, VkApiClient vk) throws ClientException, ApiException {
            vk.messages().send(actor)
                    .message(message)
                    .userId(userID)
                    .randomId(super.other().randomId(8000))
        ...
        //-----------------отправить сообшение в чат-----------------------------------//
        private void vkSendMessageChat(GroupActor actor, String message, int chatID, VkApiClient vk) throws ClientException, ApiException {
            vk.messages().send(actor)
                    .message(message)
                    .chatId(chatID)
                    .randomId(super.other().randomId(8000))
        ...
        public void vksendImageMessages(GroupActor actor, List<Dialog> messagesList, VkApiClient vk) throws ClientException, ApiException {
            String memPath = "./src/resources/mem/";    // беру путь
            Path path = Paths.get(memPath);             // перевожу в path
            if (Files.exists(path)) {                   // если путь существует
                File file = new File(memPath); //
